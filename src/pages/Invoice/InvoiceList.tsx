...
import { useGetAllInvoicesApi } from '../../hooks/API/InvoiceApi';
...
const InvoiceList = () => {
  ...
  const { data, isLoading, error } = useGetAllInvoicesApi(pagination.pageIndex + 1, pagination.pageSize, searchObj);
  ...
  // Define columns using modern DataTable format
  const columns = useMemo<ColumnDef<Invoice>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            onClick={() => navigate(`/invoice-list/${row.original.id}`)}
          >
      ...
      {
        accessorKey: 'vendor.tempName',
        header: 'Vendor Name',
        cell: ({ getValue }) => <div className="font-medium">{getValue() as string}</div>,
      },
      {
        id: 'linkedReferences',
        header: 'Linked GRN / PO',
        cell: ({ row }) => (
          <span className="text-gray-700 dark:text-gray-300">
            {`${row.original.grn?.grnNumber || 'N/A'} / ${row.original.po?.purchaseOrderNumber || 'N/A'}`}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => {
          const amount = (row.original.totalAmount || 0) + (row.original.totalTaxAmount || 0);
          return (
  ...
  const handleExport = useCallback(() => {
    if (!mockData.invoices) return;
    
    const selectedRows = Object.keys(rowSelection).map(rowIndex => 
      mockData.invoices[parseInt(rowIndex, 10)]
    );
  ...
  }, [rowSelection, mockData.invoices]);
  ...
  return (
    <div className="p-6">
      <PageHeader 
        title="Invoice" 
        subtitle="Manage your Invoice processes and vendor responses"
        showBackButton={true}
      >
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline">Filters</Button>
          <Button size="sm" variant="primary" onClick={() => navigate('/invoice/create')}>
            + Create Invoice
          </Button>
        </div>
      </PageHeader>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="flex space-x-1 rounded-lg bg-gray-100 p-1 dark:bg-gray-800">
            {BuyersTopTabsData.map((label) => {
              const count = label === 'All' ? (mockData.totalCount || 0) : 
                mockData.invoices?.filter((invoice: Invoice) => invoice.status === label.toUpperCase()).length || 0;
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        <DataTable
          ...
          totalItems={mockData.totalCount || 0}
          isLoading={isLoading}
