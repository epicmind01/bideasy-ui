...
const ASNList = () => {
  ...
  const { data: asnData, isLoading } = useGetAdvanceSimpmentNoteList(
  ...
  // Define columns
  const columns = useMemo<ColumnDef<AdvanceSimpmentNote>[]>(
    () => [
      {
        id: 'select',
        header: ({ table }) => (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            onClick={() => navigate(`/asn-list/${row.original.id}`)}
          >
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-gray-700 dark:text-gray-300">
            {row.original.purchaseOrder?.purchaseOrderNumber || 'N/A'}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-gray-700 dark:text-gray-300">
            {row.original.GoodReceiptNote && row.original.GoodReceiptNote.length > 0 ? row.original.GoodReceiptNote[0].grnNumber : 'N/A'}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-gray-700 dark:text-gray-300">
            {row.original.ewayBillNumber || 'N/A'}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-gray-700 dark:text-gray-300">
            {row.original.acknowledgedBy || 'N/A'}
          </span>
        ),
      },
      {
  ...
  const handleExport = useCallback(() => {
    if (!asnData?.advanceSimpmentNotes) return;
    
    const selectedRows = Object.keys(rowSelection).map(rowIndex => 
      asnData.advanceSimpmentNotes[parseInt(rowIndex, 10)]
    );
  ...
  }, [rowSelection, asnData?.advanceSimpmentNotes]);
  ...
  return (
    <div className="p-6">
      <PageHeader 
        title="ASN List" 
        subtitle="Manage your Advance Shipping Notices and track shipments"
        showBackButton={true}
      >
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="flex space-x-1 rounded-lg bg-gray-100 p-1 dark:bg-gray-800">
            {[
              ...
              const count = (asnData?.[key.toLowerCase() as keyof typeof asnData] as number) ?? 0;
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        <DataTable
          columns={columns}
          data={asnData?.advanceSimpmentNotes || []}
          pagination={{
            pageIndex: pagination.pageIndex,
            pageSize: pagination.pageSize,
            total: asnData?.totalCount || 0
          }}
