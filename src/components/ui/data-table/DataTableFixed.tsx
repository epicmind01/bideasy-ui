...
// DataTableFixed component - same as DataTable but with fixed layout
interface DataTableFixedProps<TData> {
...
export function DataTable<TData>({
  ...
  const [pagination, setPagination] = useState<PaginationState>({
    pageIndex: externalPagination?.pageIndex ?? externalCurrentPage - 1, // 0-based for internal use
    pageSize: externalPagination?.pageSize ?? externalPageSize,
  });
  ...
  const table = useReactTable({
    ...
    pageCount: Math.ceil((externalPagination?.total ?? totalItems) / pagination.pageSize),
  ...
  const actualTotalItems = externalPagination?.total ?? totalItems;
  const pageCount = Math.ceil(actualTotalItems / pagination.pageSize);
        ...
        <div className="flex items-center space-x-2">
          <span className="text-sm text-gray-700 dark:text-gray-300">
            ...
            <span className="font-medium">
              {Math.min((pagination.pageIndex + 1) * pagination.pageSize, actualTotalItems)}
            </span>{' '}
            of <span className="font-medium">{actualTotalItems}</span> results
          </span>
        </div>
