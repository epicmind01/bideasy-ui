...
interface CreateManualPOModalProps {
  isOpen: boolean;
  onClose: () => void;
  prId: string;
  originalItems: any[];
...
const CreateManualPOModal: React.FC<CreateManualPOModalProps> = ({ 
  isOpen,
  onClose,
  prId, 
  originalItems, 
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await createManualPO.mutateAsync({
        prId,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">Create Manual PO from Approved PR</h2>
          <button
            onClick={handleClose}
        ...
        <div className="flex justify-end space-x-3">
          ...
          >
            {isSubmitting ? 'Creating...' : 'Create Manual PO'}
          </button>
        </div>
      </div>
    </div>
...
export default CreateManualPOModal;
